...
exports.editOption = catchAsync(async(req , res , next)=>{
    ...
    if(body.maximumCartItems){
        isIntegerValidWithError(body.maximumCartItems , 10000 , 1 , "maximumCartItems" , errors);
        eOBody.maximumCartItems = body.maximumCartItems;
    };

    if(body.maximumOrdersPerUser){
        isIntegerValidWithError(body.maximumOrdersPerUser , 10000 , 1 , ",maximumOrdersPerUser" , errors);
        eOBody.maximumOrdersPerUser = body.maximumOrdersPerUser;
    };

    if(body.shopStatus){
        if(typeof body.shopStatus != "string")
            errors.push({fieldName:"shopStatus" , errMessage:`نوع البيانات ${body.shopStatus} غير صحيح`});

        if(!["مغلق" , "مفتوح"].includes(body.shopStatus))
            errors.push({fieldName:"shopStatus" , errMessage:`القيمة ${body.shopStatus} غير صالحة يجب أن تكون القيمة إما مغلق أو مفتوح`});

        eOBody.shopStatus = body.shopStatus;
    };

    if(body.dailyMessageLimit){
        isIntegerValidWithError(body.dailyMessageLimit , 10 , 1 , "dailyMessage" , errors);
        eOBody.dailyMessageLimit = body.dailyMessageLimit;
    };

    if(body.monthlyMessageLimit){
        isIntegerValidWithError(body.monthlyMessageLimit , 1000 , 1 , "monthlyMessage" , errors);
        eOBody.monthlyMessageLimit = body.monthlyMessageLimit;
    };

    
    if(errors.length > 0)
    throw new error(errors , 1225 , 400 , true);
    
    if(req.file){
        req.file.fileName = `${randomString(10)}_${new Date().toISOStringV2().replace(/:/g , "-")}.${req.file.type}`;
