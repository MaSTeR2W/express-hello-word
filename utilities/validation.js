...
const isExceededOrderLimit = async(product , quantity , checkLimit = true)=>{
    ...
    if(quantity > product.quantity)
        return {
            fieldName:"quantity",
            errMessage:`الكمية المطلوبة (${quantity}) أكثر من الكمية المتوفرة (${product.quantity || 0}).`
        };
...
exports.isIntegerValid = (number , biggestValue , lowestValue , fieldName)=>{
    if(!Number.isInteger(number-0))
        return {
            fieldName, errMessage:`القيمة ${number} غير صالحة`
        };

    if(number > biggestValue)
        return {
            fieldName , errMessage:`القيمة ${number} أكبر من ${biggestValue}.`
        };

    if(number < lowestValue)
        return{
            fieldName, errMessage:`القيمة ${number} أصغر من ${lowestValue}`
        };
};

exports.isIntegerValidWithError = (number , biggestValue , lowestValue , fieldName , errors)=>{
...
exports.notificationValidator = (body , title , content)=>{
    ...
    if(errors.length > 0)
        throw new error(errors , 1287 , 400 , true);
